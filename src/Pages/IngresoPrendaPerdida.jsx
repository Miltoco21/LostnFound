...
const IngresoPrendaPerdida = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/prendas/buscar`, {
        params: {
          rut: searchRut
        },
        timeout: 30000,
  ...
  // Función para confirmar la devolución - CORREGIDA
  const handleConfirmReturn = async () => {
    if (!selectedGarment || !returnStatus) {
      showAlert("Por favor seleccione un estado válido", "warning");
      return;
    ...
    try {
      ...
      // ✅ CAMBIO: Usar PATCH en lugar de PUT para el endpoint correcto
      const response = await fetch(`${baseUrl}/prendas/${selectedGarment.id}/estado-devolucion`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
